...
public class SubGoalIntegrationTest {
    ...
    @Test
    @DisplayName("권한이 없는 사용자가 서브 목표를 생성하려고 하면 에러가 발생한다")
    void 권한이_없는_사용자가_서브_목표를_생성하려고_하면_에러가_발생한다() throws Exception {
        //given
        ProjectEntity project = createProject("tbntb-1", "프로젝트 제목", "프로젝트 설명");
        ProjectEntity savedProject = projectRepository.save(project);
        Long projectId = savedProject.getId();
    ...
    @Test
    @DisplayName("권한이 없는 사용자가 서브 목표를 삭제하려고 하면 에러가 발생한다.")
    void 권한이_없는_사용자가_서브_목표를_삭제하려고_하면_에러가_발생한다() throws Exception {
        //given
        ProjectEntity project = createProject("tbntb-1", "프로젝트 제목", "프로젝트 설명");
        ProjectEntity savedProject = projectRepository.save(project);
        Long projectId = savedProject.getId();
